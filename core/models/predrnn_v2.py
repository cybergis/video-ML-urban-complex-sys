...
class RNN(nn.Module):
    ...
    def forward(self, frames_tensor):
        ...
        frames = frames_tensor.contiguous()#.permute(0, 1, 4, 2, 3).contiguous()
        # mask_true = mask_true.contiguous()#.permute(0, 1, 4, 2, 3).contiguous()
        ...
        next_frames = torch.stack(next_frames, dim=0).contiguous()#.permute(1, 0, 3, 4, 2).contiguous()
        # loss = self.MSE_criterion(next_frames, frames_tensor[:, 1:]) + self.configs.decouple_beta * decouple_loss
        return next_frames#[:, 1:]#, loss
