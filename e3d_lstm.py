...
class E3DLSTM(nn.Module):
    def __init__(self, input_shape, hidden_size, num_layers, kernel_size, tau):
        ...
        self.decode = nn.Conv3d(hidden_size, input_shape[0], (1, 5, 5), padding = (0, 2, 2))
    ...
    def forward(self, input):
        ...
        return self.decode(torch.cat(outputs, dim=1))#[1:]
...
class E3DLSTMCell(nn.Module):
    ...
    def forward(self, x, c_history, m, h):
        ...
        # R is CxT×H×W
        # print(x.shape, input.shape[-3:])
        # print(self.weight_xr(x).shape, self.weight_xr(x).shape)
        r = torch.sigmoid(LR(self.weight_xr(x) + self.weight_xr(x)))
...
class ConvDeconv3d(nn.Module):
    ...
    def forward(self, input):
        ...
        # print(input.shape, self.conv3d(input).shape)
